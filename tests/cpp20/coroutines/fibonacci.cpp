...
/*
co_await not always will wait/suspend. depending on your decision in bool
await_ready();

std::coroutine_handle - raw pointer to coroutine frame
std::coroutine_handle can:
void resume() - resume suspended coroutine
void destroy() - clean up/delete coroutine frame not reached end (when it will
be cleaned automatically) promise_type& promise() const - get promise from
coroutine static coroutine_handle from_promise(promise_type&) - create handler
from promise std::coroutine_handle is not a shared_ptr - plain value!
...
coroutine has access only to Awaitable and that is the way to pass something out
to caller so in co_await Fibonacci::Awaiter{XXX} we can send XXX to Awaiter to
store it later Awaiter can pass this data to Promise in await_suspend

from caller we have Fibonacci, from it's c_handle we can get promise and get
that value

To coroutine:
Fibonacci -> c_handle -> promise -> resume -> Awaiter -> XXX await_resume() {
return promise().XXX; }

instead of returning void in await_suspend you can return another coroutine
handler to resume another coroutine instead of returning back to caller after
that you will return to original coroutine
*/
...
struct Fibonacci {
    struct Promise {
        ...
        Fibonacci get_return_object() noexcept {
            ...
            // this needed to get std::coroutine_handle in ReturnType so I can
            // operate with it if needed
        ...
        // ctor - parameters will be exactly what is send to coroutine function
    ...
    struct Awaitable {
        ...
        // can be std::coroutine_handle<> (points on any coroutine) or
        // std::coroutine_handle<promise_type> (exact that type of coroutine)
        // instead of returning void you can return another coroutine handler to
        // resume another coroutine instead of returning back to caller
        ...
        Awaitable(int i) : value(i) {}
    };

    std::coroutine_handle<Promise> c_handle;

    void resume() { c_handle.resume(); }

    int getValue() { return c_handle.promise().value; }
};
...
TEST_CASE("Fibonacci") {
    ...
    // start coroutine
    f_coro.resume();
    // we will get here as soon as coroutine will be paused (in our case by
    // co_yield)
