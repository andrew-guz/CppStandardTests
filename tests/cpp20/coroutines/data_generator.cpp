...
struct DataGenerator {
    struct promise_type {
        ...
        void return_value(const std::string msg) noexcept { data = std::move(msg); }
        void return_value(const int i) noexcept { data = i; }
    };

    std::coroutine_handle<promise_type> coroutine_handler;

    explicit DataGenerator(promise_type* p) : coroutine_handler(std::coroutine_handle<promise_type>::from_promise(*p)) {}

    DataGenerator(DataGenerator&& other) : coroutine_handler(std::exchange(other.coroutine_handler, nullptr)) {}
    ...
    bool isDone() const noexcept { return coroutine_handler.done(); }
