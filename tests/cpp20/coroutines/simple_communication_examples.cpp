...
struct YieldReturn {
    ...
    void resume() { _co_handle.resume(); }

    int value() { return _co_handle.promise()._value; }
};

YieldReturn TestYieldReturn() { co_yield 5; }
...
CoReturn TestCoReturn() { co_return 7; }
...
struct AwaitReturn1 {
    ...
    struct Awaiter {
        ...
        void await_suspend(std::coroutine_handle<Promise> h) noexcept { h.promise()._value = _value; }
...
AwaitReturn1 TestAwaitReturn1() { co_await AwaitReturn1::Awaiter{9}; }
...
AwaitReturn2 TestAwaitReturn2() { co_await 13; }
...
struct ToCoroutineAndBackReturn {
    ...
    struct Awaiter {
        ...
        bool await_ready() noexcept { return false; }
        void await_suspend(std::coroutine_handle<Promise> h) noexcept { _co_handle = h; }
        int await_resume() noexcept { return _co_handle.promise()._value; }
    };
    ...
    void setValue(int v) { _co_handle.promise()._value = v; }
