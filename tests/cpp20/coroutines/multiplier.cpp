...
struct Multiplier {
    struct Promise {
        ...
        std::suspend_always yield_value(const int m) noexcept {
            multiplication = m;
            return {};
        }
    };
    ...
    struct Awaiter {
        ...
        constexpr void await_suspend(std::coroutine_handle<Promise> h) noexcept { c_handle = h; }
        int await_resume() const noexcept { return c_handle.promise().member; }
    };

    std::coroutine_handle<Promise> c_handle;

    bool isDone() const noexcept { return c_handle.done(); }

    void resume() { c_handle.resume(); }

    void sendMultiplier(int val) { c_handle.promise().member = val; }

    int getResult() { return c_handle.promise().multiplication; }
};
